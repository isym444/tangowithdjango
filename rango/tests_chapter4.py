...
class Chapter4ExerciseTests(TestCase):
    ...
    def test_about_contains_required_text(self):
        ...
        for required_str in required:
            self.assertTrue(required_str in self.about_response.content.decode(), f"{FAILURE_HEADER} exp: '{required_str}' got: '{self.about_response.content}' .{FAILURE_FOOTER}")
    ...
    def test_about_contains_rango(self):
        ...
        self.assertTrue(required_str in self.about_response.content.decode(), f"{FAILURE_HEADER}The Heck the book.{FAILURE_FOOTER}")
    ...
    def test_about_contains_cat(self):
        ...
        print(repr(required_pattern))
        self.assertTrue(re.search(required_pattern, self.about_response.content.decode()), f"{FAILURE_HEADER} exp: {required_pattern}. got: {self.about_response.content.decode()} {FAILURE_FOOTER}... regex: {re.search(required_pattern, self.about_response.content.decode())}")